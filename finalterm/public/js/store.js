...
function displayProducts() {
    var storeId = localStorage.getItem("storeId");
    $.ajax({
        url:  `http://localhost:3000/api/products?storeId=${storeId}`,
        method: "GET",
        dataType: "json",
        success: function(data) {
...
function handleProductFormSubmission(event) {
    ...
    if (productId) {
        $.ajax({
            url: `http://localhost:3000/api/products/${productId}?storeId=${storeId}`,
            method: "PUT",
            contentType: "application/json",
        ...
        $.ajax({
            ...
            data: JSON.stringify(product),
            success: function() {
                window.location.href = "/store"; // Redirect to the store page after creating the product
            },
            error: function(error) {
...
$(document).ready(function() {
    ...
    $(".addProductForm").submit(handleProductFormSubmission);
